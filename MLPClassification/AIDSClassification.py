...
class NeuralNetwork:
    def __init__(self, data_path, train: bool = True):
        ...
        self.classifier = MLPClassifier(hidden_layer_sizes=(100,50), activation='tanh', solver='adam', max_iter=1000, random_state=42, learning_rate_init=0.001)
    ...
    def evaluate_emoji(self): 
        ...
        y_pred = " ".join(list(map(lambda x: "💀" if x == 1 else "🔥", y_pred)))
        y_pred = "\n".join(y_pred[i:i+20] for i in range(0, len(y_pred), 20))
        print(y_pred)
    ...
    def save_model(self, path="./models_weights/AIDS_Classification_model.pkl"):
        # Save the model
        joblib.dump(self.classifier, path)
        print("Model saved successfully!")

    def load_model(self, path="./models_weights/AIDS_Classification_model.pkl"):
